...
async function channels(message) {
    ...
    const channelList = client.channels.cache.array();
    const statsChannel = client.channels.cache.get(DISCORD_STATS_CHANNEL);
...
async function count(message, args) {
    ...
    try {
        const { guild } = message;
        const channelToCount = guild.channels.cache.find((channel) => channel.name === channelRequestedToCount);
        const channelName = channelToCount.name;
        const channelMemberCount = channelToCount.members.size;
