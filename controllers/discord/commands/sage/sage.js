...
let usdtBtcSovMining = async (message, args) => {
    ...
    const response = await axios.get(`https://backend.sovryn.app/amm/liquidity-mining/${walletAddress}`);
    console.log(response.data); 
    const exampleEmbed = new Discord.MessageEmbed()
    .setColor('#0099ff')
    .setTitle('Liquidity Mining: USDT/BTC Pool')
    .addFields(
        { name: 'Asset: USDT', value: `Your Current Share Of Reward Pool*: ${response.data[0].percentage}%\nYour Current Share Of Reward Pool*: ${response.data[0].sovReward}`, inline: true },
        { name: 'Asset: BTC', value: `Your Current Share Of Reward Pool*: ${response.data[1].percentage}%\nYour Current Share Of Reward Pool*: ${response.data[1].sovReward}`, inline: true },
    );
