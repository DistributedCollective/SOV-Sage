...
async function channels(message, args) {
	...
	const channelsEmbed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('Channel List w/ Member Count');
...
async function count(message, args) {
    try {
        ...
        await message.channel.send(`Counting members in: ${channelName}, there are ${channelMemberCount} connected.`);
    } catch (err) {
